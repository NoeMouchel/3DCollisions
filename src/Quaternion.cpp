...
ToolBox::Quaternion::Quaternion(float in_x, float in_y, float in_z, float in_w)
{
    x = in_x;
    y = in_y;
    z = in_z;
...
ToolBox::Quaternion::Quaternion(Vector3f v, float in_w)
{
    x = v.x;
    y = v.y;
    z = v.z;
...
ToolBox::Quaternion::Quaternion(Vector4f v)
{
    x = v.x;
    y = v.y;
    z = v.z;
...
//  Functions
float ToolBox::Quaternion::length() const
{
    return sqrtf(x * x + y * y + z * z + w * w);
}

float ToolBox::Quaternion::squareLength() const
{
    return x * x + y * y + z * z + w * w;
}

ToolBox::Quaternion ToolBox::Quaternion::normalized() const
{
    float len = length();
...
ToolBox::Quaternion ToolBox::Quaternion::invert() const
{
...
void ToolBox::Quaternion::toAxisAngle(Vector3f& outAxis, float& outAngle)
{
...
ToolBox::Vector3f ToolBox::Quaternion::toEuler()
{
    Vector3f angleDEG =
    { 
        atan2f(2.0f * (w * x + y * z), 1.0f - 2.0f * (x * x + y * y)),
        asinf(ToolBox::clamp(2.0f * (w * y - z * x), -1.f, 1.f)),
...
// !! NOT VERIFIED YET
ToolBox::Mat4x4 ToolBox::Quaternion::toMatrix()
{
...
ToolBox::Quaternion ToolBox::quaternionFromVector3ToVector3(Vector3f from, Vector3f to)
{
...
ToolBox::Quaternion ToolBox::quaternionFromAxisAngle(const Vector3f& axis, float angle)
{
...
ToolBox::Quaternion ToolBox::quaternionFromEuler(const float roll, const float pitch, const float yaw)
{
...
ToolBox::Quaternion ToolBox::quaternionFromEuler(const Vector3f& euler)
{
    return quaternionFromEuler(euler.x, euler.y, euler.z);
}


ToolBox::Quaternion ToolBox::operator*(const Quaternion& q, Mat4x4 mat)
{
...
ToolBox::Vector3f ToolBox::vector3RotateByQuaternion(const Vector3f& v, const Quaternion& q)
{
    return
    {
...
// !! NOT VERIFIED YET
ToolBox::Quaternion ToolBox::Mat4x4::toQuaternion()
{
    Quaternion result;
