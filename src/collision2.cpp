...
bool ToolBox::intersectionPointPoint(Vector2f point_A, Vector2f point_B)
{
...
bool ToolBox::intersectionPointLine(Vector2f point, Line line)
{
...
bool ToolBox::intersectionPointSegment(Vector2f point, Segment seg)
{
...
bool ToolBox::intersectionPointCircle(Vector2f point, Circle cir)
{
...
bool ToolBox::intersectionPointBox(Vector2f point, Box box)
{
...
bool ToolBox::intersectionLineLine(Line line_A, Line line_B)
{
...
bool ToolBox::intersectionLineSegment(Line line_A, Segment segment)
{
...
bool ToolBox::intersectionLineCircle(Line line, Circle circle)
{
...
bool ToolBox::intersectionSegmentSegment(Segment segment_A, Segment segment_B)
{
...
bool ToolBox::intersectionSegmentCircle(Segment segment, Circle circle)
{
...
bool ToolBox::intersectionCircleCircle(Circle circle_A, Circle circle_B)
{
...
bool ToolBox::intersectionCircleBox(Circle circle, Box box)
{
    if (intersectionPointBox(circle.o, box))
        return true;
    
    //nearest point of the box from the O() of circle
    Vector2f h(ToolBox::clamp(circle.o.x, box.o.x - box.width / 2, box.o.x + box.width / 2), ToolBox::clamp(circle.o.y, box.o.y - box.length / 2, box.o.y + box.length / 2));
...
bool ToolBox::intersectionBoxBox(Box box_A, Box box_B)
{
...
bool ToolBox::intersectionConvexPoint(ConvexPolygon convex, Vector2f point)
{   
    convex.makeSegments();
...
bool convexConvexTest(ToolBox::ConvexPolygon convex1, ToolBox::ConvexPolygon convex2)
{
    for (int i = 0; i < (int)convex1.segments.size(); i++)
    {
        ToolBox::Vector2f normal = convex1.segments[i].getNormal();
        ToolBox::Range2 current_range1 = range2Segment(convex1.segments[0], normal);
        for (int j = 1; j < (int)convex1.points.size(); j++)
            current_range1 = current_range1 + range2Segment(convex1.segments[j], normal);
      
        ToolBox::Range2 current_range2 = range2Segment(convex2.segments[0], normal);
...
bool ToolBox::intersectionConvexConvex(ConvexPolygon convex1, ConvexPolygon convex2)
{   
...
bool ToolBox::intersectionConvexCircle(ConvexPolygon convex, Circle circle)
{   
    convex.makeSegments();
